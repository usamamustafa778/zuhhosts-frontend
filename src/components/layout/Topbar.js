...
export default function Topbar({ onMenuToggle }) {
  ...
  const { activeSubscription, hasActiveSubscription, loadActiveSubscription } =
  ...
  const showHostSwitcher =
    isSuperAdmin || user?.originalRole === "superadmin" || isImpersonating;
  ...
  const fetchHosts = async () => {
    ...
    try {
      ...
      console.log(
        "ℹ️ Could not fetch hosts list (this is normal if not superadmin)",
      );
      setHosts([]);
    } finally {
  ...
  const handleStopImpersonation = async () => {
    ...
    try {
      ...
      alert(
        error.message ||
          "Failed to return to superadmin view. Please try again.",
      );
    } finally {
      setSwitchingHost(false);
    }
  ...
  return (
    <header className="sticky top-0 z-30 bg-slate-900 border-b border-slate-900">
      ...
      <div className="lg:hidden">
        <div className="flex items-center justify-between px-4 py-4 bg-slate-900 text-white">
          {/* Left: Menu */}
          <button
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full active:bg-slate-800 transition-colors"
          ...
          <div className="flex-1 flex items-center justify-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-linear-to-br from-rose-500 to-pink-600 shadow-sm">
              <Building2 className="w-4 h-4 text-white" strokeWidth={2.5} />
            </div>
            <h1 className="text-lg font-bold text-white">ZuhHosts</h1>
          </div>
            ...
            {isAuthenticated && (
              <div className="relative" ref={currencyRef}>
                <button
                  ...
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-slate-700 bg-slate-800 text-sm font-medium text-slate-100 shadow-sm active:scale-95 transition-transform disabled:opacity-50"
              ...
              <>
                ...
                {tenant?.name && (
                  <div className="min-w-0 max-w-[120px]">
                    ...
                    {tenant.businessType && (
                      <p
                        className="truncate text-xs text-slate-500 capitalize"
                        title={tenant.businessType}
                      >
      ...
      {/* Desktop Topbar */}
      <div className="hidden lg:block px-8 py-1.5 bg-slate-900 text-white">
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {activeSubscription.status === "approved" &&
                    activeSubscription.maxProperties && (
                      <span className="text-xs text-slate-500">
                        •{" "}
                        {activeSubscription.maxProperties === -1
                          ? "Unlimited"
                          : activeSubscription.maxProperties}{" "}
                        properties
                      </span>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden md:flex items-center gap-2 mr-3">
            ...
            <div className="leading-tight">
              <p className="text-sm font-semibold text-white">Zuha Hosts</p>
              <p className="text-[11px] text-slate-300">Property Management</p>
            </div>
          </div>

          <div className="relative flex-1 max-w-2xl mx-auto" ref={searchRef}>
            <div className="relative flex h-10 items-center rounded-full border border-slate-700 bg-slate-800 px-4 text-sm text-slate-100 focus-within:ring-2 focus-within:ring-slate-500">
              <svg
              ...
              <input
                ...
                className="ml-3 flex-1 bg-transparent focus:outline-none placeholder:text-slate-400"
              />
              {isSearching && (
                <div className="ml-2 h-4 w-4 animate-spin rounded-full border-2 border-slate-600 border-t-slate-300" />
              )}
            </div>
                    ...
                    <div className="px-4 py-2 border-t border-slate-100">
                      ...
                      {searchResults.tasks.map((task) => (
                        <button
                          ...
                          onClick={() =>
                            handleSearchResultClick("task", task.id || task._id)
                          }
                          className="w-full text-left px-3 py-2 rounded-lg hover:bg-slate-50 transition-colors mb-1"
                        >
                          <p className="text-sm font-medium text-slate-900">
          ...
          {isAuthenticated && (
            <div className="relative" ref={currencyRef}>
              <button
                onClick={() => setIsCurrencyOpen((prev) => !prev)}
                disabled={isCurrencyLoading}
                className="flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-800 px-3 py-1.5 text-sm font-medium text-slate-100 shadow-sm transition hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-500 disabled:opacity-50"
                aria-label="Select currency"
              >
          ...
          {showHostSwitcher && (
            <div className="relative" ref={hostSwitcherRef}>
              <button
                ...
                disabled={switchingHost}
                className="flex items-center gap-2 rounded-full border border-slate-700 bg-slate-800 px-4 py-2 text-sm font-medium text-slate-100 shadow-sm transition hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-500 disabled:opacity-50"
              >
                <span>🏠</span>
          ...
          {isAuthenticated && user ? (
            <div className="flex items-center gap-3">
              <div className="relative" ref={profileRef}>
                <button
                  onClick={() => setIsProfileOpen((prev) => !prev)}
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-slate-700 text-white font-semibold text-sm hover:bg-slate-800 transition-colors focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-2"
                  aria-label="Profile menu"
                >
                  {getImageUrl(user.profilePicture) ? (
                    <img
              ...
              {tenant?.name && (
                <div className="max-w-[180px] shrink-0 min-w-0">
                  ...
                  {tenant.businessType && (
                    <p
                      className="truncate text-xs text-slate-300 capitalize"
                      title={tenant.businessType}
                    >
                      {tenant.businessType.replace(/_/g, " ")}
                    </p>
