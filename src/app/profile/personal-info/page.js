...
export default function PersonalInfoPage() {
  ...
  useSEO({
    ...
    description:
      "Update your personal information, contact details, and business details.",
    keywords:
      "personal information, profile details, contact information, business details, user profile",
  });
  ...
  const fetchUserData = async () => {
    try {
      ...
      const list =
        currenciesRes?.currencies ?? currenciesRes?.data?.currencies ?? [];
      setCurrencies(
        Array.isArray(list)
          ? list
      ...
      setFormData({
        ...
        defaultCurrency:
          userData.defaultCurrency || currenciesRes?.defaultCurrency || "",
      });
    } catch (err) {
      console.error("Failed to load user data:", err);
      toast.error(
        err.message || "Failed to load profile information. Please try again.",
      );
      setUser(null);
      setTenant(null);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.tenant) {
        ...
        if (typeof window !== "undefined") {
          window.dispatchEvent(
            new CustomEvent("tenant-updated", { detail: response.tenant }),
          );
        }
      }
      ...
      toast.error(
        err.message || "Failed to update information. Please try again.",
        { id: toastId },
      );
    } finally {
      setSaving(false);
  ...
  if (isLoading || loading) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 px-4">
        <div
          className="h-12 w-12 animate-spin rounded-full border-4 border-slate-200 border-t-rose-500"
          aria-hidden="true"
        />
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="border-b border-slate-100 bg-slate-50/50 px-6 py-4 sm:px-8">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Your business name, type, and public URL
                  </p>
                </div>
              </div>
            </div>
            <div className="space-y-6 p-6 sm:p-8">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Updates your business name and public URL slug.
                </p>
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Hotel, Airbnb/vacation rental, or both.
                </p>
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Set during signup or tenant setup.
                </p>
              </div>
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="space-y-6 p-6 sm:p-8">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                Digits, +, -, (), spaces only. Leave empty to clear.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-rose-100">
                ...
                >
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0-8v1m0-1c-1.11 0-2.08.402-2.599 1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
                  />
                </svg>
              </div>
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Preferred currency for amounts
                </p>
              </div>
            </div>
          </div>
          <div className="p-6 sm:p-8">
            ...
            <p className="mt-1 text-xs text-slate-500">
              Must be a supported currency code (e.g. USD, PKR).
            </p>
          </div>
        </section>
