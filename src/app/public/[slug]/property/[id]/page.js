...
export default function PublicPropertyPage() {
  ...
  }, [bookingForm.startDate, bookingForm.endDate, bookingForm.roomId, bookingForm.unitId]);
  ...
  const checkAvailability = async () => {
    try {
      ...
      const availabilityData = await checkPublicAvailability(slug, propertyId, params);
  ...
  const calculateTotal = () => {
    ...
    if (bookingForm.roomId) {
      const selectedRoom = rooms.find((r) => (r.id || r._id) === bookingForm.roomId);
      pricePerNight = selectedRoom?.price || pricePerNight;
    }

    if (bookingForm.unitId) {
      const selectedUnit = units.find((u) => (u.id || u._id) === bookingForm.unitId);
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 rounded-2xl border-2 border-slate-200 bg-white p-6 shadow-lg">
              <h3 className="text-xl font-bold text-slate-900 mb-4">Book Your Stay</h3>
              ...
              <form onSubmit={handleBooking} className="space-y-4">
                  ...
                  <div className="bg-slate-50 rounded-lg p-4 space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-600">
                        ${" "}
                        {bookingForm.roomId
                          ? rooms.find((r) => (r.id || r._id) === bookingForm.roomId)?.price || property.price
                          : bookingForm.unitId
                          ? units.find((u) => (u.id || u._id) === bookingForm.unitId)?.price || property.price
                          : property.price}{" "}
                        × {calculateNights()} nights
                      </span>
                      <span className="font-semibold text-slate-900">${calculateTotal()}</span>
                    </div>
                ...
                <button
                  type="submit"
                  disabled={isBooking || (availability && !availability.available)}
